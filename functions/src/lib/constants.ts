...
export const REASONING_SYS_PROMPT =
  "You are an assistant that analyzes the JSON output of a receipt processor. Your task is to determine if the JSON output makes conceptual sense. If the JSON does not make sense, fix the mistake. One mistake to note is when fiscal information such as tax or subtotal, are stored in ReceiptItems.";
...
export const JSONSCHEMA: ResponseFormatV2 = {
  ...
  "jsonSchema": {
    ...
    "properties": {
      ...
      "receiptItems": {
        "type": "array",
        "description": "A list of purchased items. Exclude all summary financial and payment lines. For example: INCLUDE 'Cheese Pizza', 'Paper Towels'. EXCLUDE 'Subtotal', 'Tax', 'Total', 'Cash Paid', 'Change Due'.",
        "items": {
          "type": "object",
          "properties": {
            "itemName": {
            ...
            "details": {
              "type": "array",
              "description": "Additional details about the item, such as size, flavor, toppings, and so on",
              "items": {
                "type": "string"
              }
            }
